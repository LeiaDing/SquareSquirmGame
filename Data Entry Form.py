...
def get_countries_as_tuples():
    ...
    for li in lis:
        parts = li.text.split('-')
        tup = (parts[0].strip(), parts[1].strip())
...
def save_data():
    ...
    if fname!='' and lname!='':
      ...
      age=agespinbox.get()
      nationality=ntcombo.get()
      registered=reg_status.get()
      print(fname,lname,age)
      print(registered)
      with open("student.txt", 'a') as filesave:
        filesave.write(fname+' '+lname+' '+age+'')
      print("saved")
    else:
        messagebox.showwarning(title='warning',message="need names")
...
titlecombo=ttk.Combobox(userinfoframe,values=["Mr","Ms","Mrs","Dr"])
...
ntlabel=tkinter.Label(userinfoframe,text='nationality')
ntlabel.grid(row=2,column=0)
ntcombo=ttk.Combobox(userinfoframe,values=country)
ntcombo.grid(row=2,column=1)
...
regcheck=tkinter.Checkbutton(courseframe,text="currently registered",variable=reg_status,onvalue='registered',offvalue='not registered')
registeredlabel.grid(row=0,column=0)
regcheck.grid(row=0,column=1)
savebutton=tkinter.Button(frame,text="save",command=save_data)
